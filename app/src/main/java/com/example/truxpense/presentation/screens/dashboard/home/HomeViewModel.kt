...
class HomeViewModel @Inject constructor(
    ...
    val recentTransactions: StateFlow<List<HomeTransactionItem>> = combine(
     ...
     */
    val monthOverMonthChange: StateFlow<MonthlyChangeData> = expenseRepository.transactions.map { list ->
        ...
        val prevWindowEnd = Calendar.getInstance().apply {
            ...
            set(Calendar.MONTH, prevMonth)
            set(
                Calendar.DAY_OF_MONTH, minOf(
                    todayDay, prevCal.getActualMaximum(Calendar.DAY_OF_MONTH)
                )
            )
        ...
        val pct = if (prevMonthSpend > 0) ((thisMonthSpend - prevMonthSpend) / prevMonthSpend) * 100.0
        else null
    ...
    val monthlyIncome: StateFlow<Double> = incomeRepository.totalIncomeBetween(monthStartMs(), monthEndMs())
        .stateIn(viewModelScope, SharingStarted.Eagerly, 0.0)

    /** Running total of all savings entries recorded so far. */
    val monthlySavings: StateFlow<Double> =
        savingsRepository.totalSavings.stateIn(viewModelScope, SharingStarted.Eagerly, 0.0)

    /** Live list of savings goals — drives the home-screen savings card. */
    val savingsGoals: StateFlow<List<SavingsGoalUi>> =
        savingsRepository.goals.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000), emptyList())

    val expenseCount: StateFlow<Int> =
        expenseRepository.transactions.map { it.size }.stateIn(viewModelScope, SharingStarted.Eagerly, 0)
     ...
     */
    val dailySpendPoints: StateFlow<FloatArray> = expenseRepository.transactions.map { list ->
        val cal = Calendar.getInstance()
    ...
    val spendingInsight: StateFlow<SpendingInsight> = combine(
        ...
        if (last7.isNotEmpty()) {
            ...
            if (maxTx != null) {
                ...
                if (avg > 0 && maxTx.amount > avg * 3 && maxTx.amount > 1000) {
                    ...
                    val dateStr = "${d.get(Calendar.DAY_OF_MONTH)} ${
                        d.getDisplayName(
                            Calendar.MONTH, Calendar.SHORT, Locale.getDefault()
                        )
                    }"
        ...
        val topCat = byCat.maxByOrNull { it.value }
        SpendingInsight(
            message = if (topCat != null) "Your top spending category this month is ${topCat.key} (₹${topCat.value.toLong()})."
            else "Keep tracking your expenses to unlock personalised insights.",
    ...
    suspend fun getBudgetDetailArgs(category: String): Triple<String, Double, Double>? {
        ...
        val budget = budgets.firstOrNull { it.category.equals(category, ignoreCase = true) } ?: return null
        ...
        val spent = txns.filter { it.category.equals(category, ignoreCase = true) && it.timestamp >= monthStart }
            .sumOf { it.amount }
